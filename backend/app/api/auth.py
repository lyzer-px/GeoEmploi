...
async def login_for_access_token(form_data: Annotated[OAuth2PasswordRequestForm, Depends()]):
    ...
    token = create_access_token(user.email, user.id, timedelta(minutes=AUTH_TOKEN_EXPIRE_MINUTES))
...
def create_access_token(email: str, user_id: int, expires_delta: timedelta):
    encode = {"sub": email, "user_id": user_id, "exp": datetime.now(datetime.timezone.utc)}
